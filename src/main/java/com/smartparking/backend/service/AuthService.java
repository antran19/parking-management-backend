...
public class AuthService {
     ...
     * Nếu user chưa tồn tại → tự động tạo tài khoản DRIVER mới.
     * Nếu đã tồn tại → đăng nhập luôn.
     */
    @Transactional
    public LoginResponse oauth2Login(OAuth2LoginRequest request) {
        String email = request.getEmail();
        String provider = request.getProvider(); // "google" hoặc "facebook"

        // Tìm hoặc tạo user
        User user = userRepository.findByEmail(email).orElseGet(() -> {
            // Tạo user mới với mật khẩu ngẫu nhiên (user OAuth không cần mật khẩu)
            ...
            User newUser = User.builder()
                    ...
                    .build();
            userRepository.save(newUser);
            log.info("Tạo tài khoản OAuth2 mới: {} (provider={})", email, provider);
            return newUser;
        });
        ...
        log.info("OAuth2 login: {} (provider={}, role={})", user.getEmail(), provider, user.getRole());
